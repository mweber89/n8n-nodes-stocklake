...
import {
  earningsFields,
  earningsOperations,
} from "./descriptions/EarningsDescription";
import {
  marketFields,
  marketOperations,
} from "./descriptions/MarketDescription";
import { newsFields, newsOperations } from "./descriptions/NewsDescription";
import {
  screenerFields,
  screenerOperations,
} from "./descriptions/ScreenerDescription";
import {
  sectorFields,
  sectorOperations,
} from "./descriptions/SectorDescription";
import {
  signalFields,
  signalOperations,
} from "./descriptions/SignalDescription";
import { stockFields, stockOperations } from "./descriptions/StockDescription";
import {
  watchlistFields,
  watchlistOperations,
} from "./descriptions/WatchlistDescription";
...
export class Stocklake implements INodeType {
  description: INodeTypeDescription = {
    ...
    properties: [
      {
        displayName:
          "A Stocklake API key is optional for Stock/Screener/Market/Earnings-calendar operations (25 calls/day without one, 200/day with a free key). Every other operation — AI research, insider activity, signals, sector/market intelligence, news feed, watchlist — requires a Pro key. Get one free at stocklake.dev/login.",
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!this.continueOnFail()) {
        const { toolName, toolArguments } = buildToolCall.call(
          this,
          resource,
          operation,
          i,
        );
        const responseData = await stocklakeApiRequest.call(
          this,
          toolName,
          toolArguments,
          i,
  ...
  switch (`${resource}.${operation}`) {
    ...
    case "stock.getMany":
      return {
        toolName: "get_stocks",
        toolArguments: {
          symbols: parseSymbolList(
            this.getNodeParameter("symbols", i, "") as string,
          ),
        },
      };
    ...
    case "market.getMovers":
      return {
        ...
        toolArguments: cleanParams({
          ...
          limit: this.getNodeParameter("limit", i, 10) as number,
          min_market_cap_b:
            (this.getNodeParameter("minMarketCapB", i, 0) as number) ||
            undefined,
        }),
      };
    ...
    case "sector.getIntelligence":
      return {
        ...
        toolArguments: cleanParams({
          sector: this.getNodeParameter("sector", i, "") as string,
          sort_by_strength: this.getNodeParameter(
            "sortByStrength",
            i,
            false,
          ) as boolean,
    ...
    case "earnings.getIntelligence":
      return {
        ...
        toolArguments: cleanParams({
          ...
          sector: this.getNodeParameter("sector", i, "") as string,
          min_ai_score:
            (this.getNodeParameter("minAiScore", i, 0) as number) || undefined,
    ...
    case "news.getFeed":
      return {
        ...
        toolArguments: {
          min_signal_score: this.getNodeParameter(
            "minSignalScore",
            i,
            60,
          ) as number,
